...
@AllArgsConstructor
@Slf4j
public class UpdateOrderFromShopTaskService {
     ...
     */
    public void updateOrderFromShopByJobRequest(Job job, OrderJobDto.Request.UpdateJob updateJobDto) throws IOException, GeneralSecurityException {
        log.debug("Call collectOrderFromShopByJobRequest - jobId:{}", job.getId());
        //DB에 상태 저장을 위해 row를 생성한다.(중복된 잡이 있는지 체크해야 한다. 요청ID와 JobID 복합)
        jobTaskService.checkAndChangeJobTask(job.getId(), updateJobDto.getShopAccount().getRequestId(), job.getJobType(),Job.ExcuteStatus.IN_PROGRESS, false, null);
        //Reebonz의 경우 WebToken을 이용하므로 SESSION타입이므로 동일하게 사용한다.
        String token = shopAccountService.getToken(
                updateJobDto.getShopAccount().getShopType(),
                updateJobDto.getShopAccount().getLoginId(),
                updateJobDto.getShopAccount().getPassword(),
                ShopAccountToken.Type.SESSION
        );

        //각 서비스 타입별로 맞는 등록 서비스를 호출한다. async를 호출하고 callback을 받아서 처리한다.
        ShopService shopService = shopServiceFactory.getShopService(updateJobDto.getShopAccount().getShopType());
        shopService.updateOrderToShop(token, job.getId(), updateJobDto).addCallback(this::resultUpdateOrderProcess, (e) -> {
    ...
    public void resultUpdateOrderProcess(OrderJobDto.Request.UpdateCallback result){
        ...
        //Callback API를 호출한다.
        ResponseEntity<String> responseEntity = internalClient.updateOrderCallback(result);
