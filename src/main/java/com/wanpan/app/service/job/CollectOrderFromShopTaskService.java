...
@AllArgsConstructor
@Slf4j
public class CollectOrderFromShopTaskService {
     ...
     */
    public void collectOrderFromShopByJobRequest(Job job, OrderJobDto.Request.CollectJob collectOrderDto) throws IOException, GeneralSecurityException {
        ...
        for(ShopAccountDto.Request request : collectOrderDto.getShopAccounts()){
            ...
            //Reebonz의 경우 수집은 BEARER타입이므로 동일하게 사용한다.
            String token = shopAccountService.getToken(request);
            ...
            shopService.collectOrderFromShop(token, job.getId(), collectOrderDto.getOrderProcessStatus(), request).addCallback(this::resultCollectOrderProcess, (e) -> {
    ...
    public void resultCollectOrderProcess(OrderJobDto.Request.CollectCallback result){
        ...
        //Callback API를 호출한다.
        ResponseEntity<String> responseEntity = internalClient.collectOrderCallback(result);
