...
@AllArgsConstructor
@Slf4j
public class CollectQnaFromShopTaskService {
     ...
     */
    public void collectQnAFromShopByJobRequest(Job job, ShopQnaJobDto.Request.CollectJob collectQnADto) throws IOException, GeneralSecurityException {
        log.debug("Call collectQnAFromShopByJobRequest - jobId:{}", job.getId());
        for(ShopAccountDto.Request request : collectQnADto.getShopAccounts()){
            ...
            shopService.collectQnAFromShop(token, job.getId(), collectQnADto.getQuestionStatus(), request).addCallback(this::resultCollectQnaProcess, (e) -> {
    ...
    public void resultCollectQnaProcess(ShopQnaJobDto.Request.CollectCallback result){
        ...
        //Callback API를 호출한다.
        ResponseEntity<String> responseEntity = internalClient.collectQnaCallback(result);
